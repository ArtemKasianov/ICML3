...
void* mediana(char* predictPairsFile, char* InPathAndPredName, char* MedianaName,int MinIter,int MaxIter) 
{
	...
	{
		//asprintf(&PredDir[iter], "iter_%d/results/predictions",iter);
		//check if exist and create
		//if (stat(PredDir[iter], &st) == -1) 
		//{
		//	printf("The is no prediction to read! Check directory %s\n",PredDir[iter]);
		//	return 0;
	...
	int NumberOfCols=0;
	//countTableElemFromFile(predictNameWithDir[0], &NumberOfCols, &NumberOfRowInPredictFile);
	//printf("Gene Pairs file %s is %d columns and %d rows...\n",predictNameWithDir[MinIter], NumberOfCols, NumberOfRowInPredictFile);
